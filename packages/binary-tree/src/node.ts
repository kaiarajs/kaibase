import { GetRowsArrayFromHeight, CreateRandomSortedIndex, DefaultCheckKeyEquality, DefaultCheckValueEquality, DefaultCompareKeysFunction, DefaultCompareValues, CreateRefArrayFromTreeHeight } from '@kairajs/core';
...
export abstract class Node<T> implements INode<T> {
    ...
    protected constructor(public options: INodeConstructor<Node<T>>) {
        ...
        this.compareValues = options.compareValues || DefaultCompareValues;
        this.checkKeyEquality = options.checkKeyEquality || DefaultCheckKeyEquality;
    ...
    public getTreeAsArrayOfArrays<T>(): any[] {
        ...
        const rows: number[] = GetRowsArrayFromHeight(height);
        // The array of arrays holding at the end all the binary elements
        const all: any[] = CreateRefArrayFromTreeHeight(height);
     ...
     * @param fn
     */
    public executeOnEveryNode(fn: any): void {
        if (this.left) {
